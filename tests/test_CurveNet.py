...
class TestCurveNet:
    ...
    @pytest.mark.parametrize(
        "initialized_curve_net,parameters",
    ...
    @pytest.mark.parametrize(
        "initialized_curve_net,base_model,parameters",
        [(param, param, param) for param in testparams],
        indirect=True,
    )
    def test_import_base_parameters(
        self, initialized_curve_net, base_model, parameters
    ):
        (
            model,
            num_classes,
    ...
    def test_init_linear(self, initialized_curve_net, base_model, parameters):
        ...
        for layer in curve_net.curve_layers:
            curve_params = [
                CurveLayer._get_curve_param_type(p) for p in layer.parameters
            ]
