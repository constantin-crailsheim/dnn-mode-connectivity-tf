...
def data_loaders(dataset: str, path: str, batch_size: int):
    """
    ...
    if dataset == "mnist":
        ...
        train_set_loader = train_set.map(standardization).shuffle(
            ds_info.splits["train"].num_examples
        )
        test_set_loader = test_set.map(standardization).shuffle(
        ...
        train_dataset = tf.data.Dataset.from_tensor_slices(
            (f[index_train, :], y[index_train])
        )
        test_dataset = tf.data.Dataset.from_tensor_slices(
            (f[index_test, :], y[index_test])
        )
