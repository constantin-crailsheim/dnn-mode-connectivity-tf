...
class CurveNet(tf.keras.Model):
    ...
    def call(self, inputs: tf.Tensor, training=None, update_batchnorm: bool = False):
        """
        ...
        # Even if training=True (happens in .fit()), do not update moving parameters of the model.
        # Only do this if 'update' is explicitly set to True.
        training = True if update_batchnorm else None
        return self.curve_model((inputs, point_on_curve_weights), training=training)
    ...
    def update_batchnorm(self, inputs: tf.Tensor, **kwargs):
        ...
        for input_ in inputs:
            ...
            self(input_, update_batchnorm=True, **kwargs)
