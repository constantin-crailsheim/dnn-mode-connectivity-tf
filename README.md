...
- We are evaluating the model with the unregularized loss, since otherwise different epochs (initialized and trained) of the curve model would not be comparable due to their different parametrization.
...
- For the MLP, we are also using the unregularized test loss during training to show that the corner points of the CurveNet correspond to the base models. In the case of the CNN, we can see this by the same accuracies of corner points and base models.
